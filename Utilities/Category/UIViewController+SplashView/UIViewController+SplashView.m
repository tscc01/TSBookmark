...
@property (nonatomic, strong) HUDProgressView *tn_viewHud;
...
- (HUDProgressView*)tn_viewHud
{
    return objc_getAssociatedObject(self, HUDProgressViewKey);
}

- (void)setTn_viewHud:(HUDProgressView *)tn_viewHud
{
    objc_setAssociatedObject(self, HUDProgressViewKey, tn_viewHud, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)showProgress
{
    ...
    self.tn_viewHud = [HUDProgressView viewFromNib];
    self.tn_viewHud.frame = self.view.frame;
    [self.view addSubview:self.tn_viewHud];
}

- (void)hideProgress
{
    if (self.tn_viewHud) {
        [self.tn_viewHud removeFromSuperview];
        self.tn_viewHud = nil;
    }
}
